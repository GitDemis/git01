  ...
  if(isset($_POST['add_user'])){
   ...
   if(empty($errors)){
       ...
       $password = sha1($password);
        ...
        $query .="name,username,password,user_level,status,image";
        $query .=") VALUES (";
        $query .=" '{$name}', '{$username}', '{$password}', '{$user_level}','1','avatar.jpg'";
        $query .=")";
        if($db->query($query)){
            $estado = 0;
          //sucess
    ...
    if ($estado == 1) {?>
        <script>
            Swal.fire({
                    ...
                    text: 'Hubo un problema al crear el usuario, revise que los campos no estén vacío.',
                    icon: 'error',
